...
const PlanDetailsPage = () => {
  ...
  const { data: existingCheckoutIntent } = useCheckoutIntent();
  const queryClientInvalidate = (userId?: number) => {
    if (!userId) {
      return;
    }
    [
      queryBffContext(userId).queryKey,
      queryBffSuccess(userId).queryKey,
